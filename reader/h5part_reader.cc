...
void h5part_reader_prep (paramfile &params, hid_t * inp, arr<int> &qty_idx,
...
void h5part_reader_finish (vector<particle_sim> &points, float thresh)
  {
  float minr=1e30;
  float minx=1e30;
  float miny=1e30;
  ...
  for (tsize i=0; i<points.size(); ++i)
    ...
    //points[i].active = 1;
    points[i].type=0;
    minr = min(minr,points[i].I);
    maxr = max(maxr,points[i].I);
...
void h5part_reader (paramfile &params, vector<particle_sim> &points)
  {
  ...
  field[0] = "/Step#0/Coords_0";
  field[1] = "/Step#0/Coords_1";
  field[2] = "/Step#0/Coords_2";
  field[3] = "/Step#0/Color_0";
  field[4] = "/Step#0/Color_1";
  field[5] = "/Step#0/Color_2";
  field[6] = "/Step#0/Radius";
  field[7] = "/Step#0/Intensity";
  float thresh = params.find<float>("thresh", 0.0);

  h5part_reader_prep (params, &file_id, qty_idx, nfields, npart, &rrr, field, &rank, &start_local);
  hid_t dataset_space;
...
// end if
  ...
  h5part_reader_finish (points, thresh);
  }
#else
