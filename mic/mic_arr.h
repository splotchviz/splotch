...
// Array class for use in offload
template<typename T>
class Array_T{
public:
	Array_T() { arr_ = NULL; size_ = 0; capacity_ = 0; allocator = &defaultAllocator;}
	...
	void operator= (Array_T<T> const& copyarr) {
	...
	void resize(int newSize) {
		if(newSize>0)
		{
			...
			{
				for(unsigned i = size_; i < newSize; i++)
				{
					new(&arr_[i]) T();
				}
				size_ = newSize;
			...
			{
				T* temp = NULL;
				temp = (T*)allocator->malloc(newSize*sizeof(T), 64);
				if(temp == NULL)
				{
	...
	T take_back() {
		if(!size_){
			printf("Array T: take_back(): Array empty\n");
			exit(1);			
		}
		else
		{
